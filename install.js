  ...
  if (platform === 'win') {
    ...
    try {
      ...
      if (res.status === 0 && res.stdout) {
        ...
        info(
          out === 'ADDED' ? `Added ${binDir} to your user PATH.` : `${binDir} already on your PATH.`
        );
      } else {
...
function runDashboard(target) {
  ...
  if (process.platform === 'win32') {
    log(
      '  1. Open a NEW terminal (or run:  $env:Path = [Environment]::GetEnvironmentVariable("Path","User"))'
    );
...
function main() {
  ...
  if (!ensureRepo(target)) {
    process.exitCode = 1;
    return;
  }
  if (!npmInstall(target)) {
    process.exitCode = 1;
    return;
  }
